    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="relative flex-1 max-w-xs">
          ...
          />
          {search && (
            <button onClick={() => doSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-300">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
              ...
              {loading ? (
                ...
                <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-900 dark:text-white">{o.user?.fullName || '---'}</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">{o.user?.email}</p>
                  </td>
                  <td className="px-4 py-3 text-gray-900 dark:text-white">{formatCurrency(o.totalAmount)}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{o.discount > 0 ? formatCurrency(o.discount) : '---'}</td>
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{formatCurrency(o.finalAmount)}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{o._count?.tickets ?? 0}</td>
                  <td className="px-4 py-3">
                    <Badge className={statusColors[o.status] || 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100'}>{o.status}</Badge>
                  </td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{formatDate(o.createdAt, 'dd/MM/yyyy')}</td>
                  <td className="px-4 py-3">
